...
title: pd.DataFrame utilities
version: 1.0
type: module
keywords: [data frame, info table]
description: |
content:
    -
remarks:
...
def info(df: pd.DataFrame,
         ...
         add = None, #  "position", "most_common_value", "negatives", "zeros", "positives", "mean", "median", "min", "max", "range", "dispersion", "iqr"
    ...
    Remarks:
    - 'most_common_ratio' is the the ratio of most common value to all no-NaN values
    ...
    #!!! TODO: use try/except to serve more types; how to serve exceptions? stop!
    ...
    if len(set(["all", "everything"]).intersection(what)) > 0:
        what.extend( [w for w in ALL if not w in what] )
        # add everything from ALL but avaid repetition
        # and preserve as much order as possible
        sub.extend(["all", "everything"])

    if len(sub) > 0:
        what = [w for w in what if not w in sub]
    ...
    INFOS = {
        "dtype": {"name": "dtype", "abbr": "dtype" , "stat": lambda ss: ss.dtype},
    ...
    for n, ss in df.iteritems():
        ...
        if (dtypes and not ss.dtype in dtypes) or \
           (exclude and ss.dtype in exclude) or (omit and n in omit):
            continue
        ...
        if len(set(["oks", "nans", "oks_ratio", "nans_ratio", "dispersion"]).intersection(what)) > 0:
            OKs = len(ss)       #!!!
            NANs = N - OKs

        if len(set(["uniques", "most_common", "most_common_ratio", "most_common_value", "dispersion"]).intersection(what)) > 0:
        ...
        for w in what:
            if w in INFOS.keys():
                ...
                try:
                    item[ what[w].__name__ ] = [ what[w](ss) ]
                except Exception as e:
                    if exceptions:
                        print(e)
                    item[ what[w].__name__ ] = [ None ]
    ...
    ## TODO: round safely (i.e. only to significant digits !!!)
    if round:
...
def summary(df: pd.DataFrame,
         ...
         add = None, #("pos", "oks", "oks_ratio", "nans_ratio", "nans", "uniques", "most_common", "most_common_ratio", "most_common_value"),
         sub = None,
         #
         omit = None,
         dtypes = None,
    ...
    rather then technical (like NaNs number and proprtions);
