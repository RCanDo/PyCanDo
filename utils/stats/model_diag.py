...
class ModelDiag(bi.Repr):
    ...
    def _get_features(self) -> None:
        """"""
        self.features = self.storage.load("xvars")

        try:
            self.best_features = self.storage.load("best_features")
        except Exception as e:
            print(e)

            if isinstance(self.model, Pipeline):
                best_features_idx = self.model['best_features_selection'].get_support(indices=True)
                best_features = np.array(self.features)[best_features_idx].tolist()
    ...
    def _get_predictions(self) -> None:
        """
        Fills attributes: self.y_train_hat, self.y_test_hat, self.y_valid_hat
        i.e. makes predictions from self.model based on train, test and valid data:
        self.X_train, self.X_test, self.X_valid.
        """
        self.y_train_hat = self.predict(self.X_train)
        self.y_test_hat = self.predict(self.X_test)
        if len(self.X_valid) > 0:
            self.y_valid_hat = self.predict(self.X_valid)
        else:
            self.y_valid_hat = pd.Series([])
    ...
    def report_md(
       ...
       """
        ...
        if self.binary and isinstance(self.metrics, MetricsBin):
            ...
            additional_content = [confusion, target_title, target]
