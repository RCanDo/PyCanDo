...
class TimeInterval():
    ...
    def intersection(self, other):
        ...
        if self.start > other.end or self.end < other.start:
            result = TimeInterval(self.start, self.start, closed=(0, 0))
        else:
            #
            start = max(self.start, other.start)
            if start == self.start:
                closed_start = self.closed[0]
            else:
                closed_start = other.closed[0]
            #
            end = min(self.end, other.end)
            if end == self.end:
                closed_end = self.closed[1]
            else:
                closed_end = other.closed[1]
            result = TimeInterval(start, end, closed=(closed_start, closed_end))
        return result
